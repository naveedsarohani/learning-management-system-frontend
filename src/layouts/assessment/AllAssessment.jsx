...
export default function Allassessment() {
  ...
  useEffect(() => {
    assessment.all(token, setAssessments, handler, user.role !== role.ADMIN && { 'course.user_id': user.id })
  }, [handler.navigate, user])

  return handler.componentLoaded && <DashboardPageCompement title={"all assessments"}>
    {!isNullOrEmpty(assessments) ? <Table
      ths={
        <>
