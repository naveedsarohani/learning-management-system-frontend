...
export function isPass({ obtained_marks, total_marks, exam: { passing_percentage } }) {
...
export default function AllResult() {
  ...
  return handler.componentLoaded && <DashboardPageCompement title={"all results"}>
    {!isNullOrEmpty(results) ? <Table
      ths={
        <>
          <th>Sno.</th>
          <th>Student</th>
      ...
      tds={!isNullOrEmpty(results) && results.map((result, index) => <tr key={result.id}>
        ...
        <td className="flex gap-2 pt-2">
          ...
          <ActionButton
            name={"Delete"}
            onClick={() => destroy("/results", result.id, "result", setResults)}
