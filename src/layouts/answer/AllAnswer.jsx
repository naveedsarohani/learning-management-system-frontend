...
export default function Allanswer() {
  ...
  return (
    <DashboardPageCompement title={"all answers"}>
          ...
          answers.map((answer, index) => (
            <tr key={answer.id}>
              ...
              {/* <td>
                ...
                <button
                  onClick={() => destroy("/answers", answer.id, "answer")}
                >
                  Delete
                </button>
              </td> */}
              ...
              <td className="flex gap-2 pt-2">
                <ActionButton route={`./${answer.id}`} name={"View"} />
                <ActionButton
                  route={`./edit/${answer.id}`}
                  name={"Edit"}
