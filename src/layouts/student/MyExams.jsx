...
export default function MyExams() {
  ...
  useEffect(() => {
    ...
    submission.all(token, setSubmissions, handler, { getOnlyProperty: 'exam_id' });
  }, [location.pathname, user])

  console.log(questions);

  return handler.componentLoaded && <div className="px-4 py-6 bg-gray-50 min-h-screen">
    <h1 className="text-2xl font-bold text-gray-900 mb-6">All Exams</h1>
    {!isNullOrEmpty(exams[0].id) ? (
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {exams.map((exam) => questions.includes(exam.id) && <ExamCard
          exam={exam}
          key={exam.id}
