...
const ExamCard = ({ exam = blueprint.exam, isAttempted }) => {
  ...
  useEffect(() => {
    getCountDown(exam.starts_at, ({ remainingTime, formattedTime: { days, hours, minutes, seconds } }) => {
      if (remainingTime) setCountDownTimer(`Starts [${days} days, ${hours}:${minutes}:${seconds}]`);
      else setCountDownTimer(false);
    });

    getCountDown(exam.starts_at, ({ remainingTime, formattedTime: { hours, minutes, seconds } }) => {
      ...
      <div className="md:flex">
        <div className="p-6">
          ...
          <div className="mt-4">
            ...
            {!countDownTimer && !isAttempted && ends !== true && <p className="text-gray-600 text-sm">
              <span className="font-bold">Ends in:</span>{" "}
              {ends}
            </p>}
          </div>
          <div className="mt-6">
            {isAttempted && <ActionButton
              name='completed'
              color="w-full py-2 bg-gray-500 text-white font-semibold text-sm rounded shadow-md cursor-not-allowed"
            />}

            {ends === true && !countDownTimer && !isAttempted && <ActionButton
              name='Exam ended'
              noCap={true}
              color="w-full py-2 bg-gray-500 text-white font-semibold text-sm rounded shadow-md cursor-not-allowed"
            />}

            {countDownTimer && ends !== true && !isAttempted && <ActionButton
              name={countDownTimer}
              noCap={true}
