...
export default function AllSubmission() {
    ...
    return <DashboardPageCompement title={'all submissions'}>
        ...
        <Table
            ...
            tds={!isNullOrEmpty(submissions) && submissions.map((submission, index) => <tr key={submission.id}>
                <td>{index + 1}</td>
                ...
                <td>
                    ...
                    <ActionButton route={`./edit/${submission.id}`} name={'Edit'} />
                    <ActionButton name={'Delete'}
                        onClick={() => destroy('/submissions', submission.id, 'submission')}
                    />
                </td>
            </tr>)}
        />
