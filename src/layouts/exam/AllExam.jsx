...
export default function AllExam() {
  ...
  useEffect(() => {
    exam.all(token, setExams, handler, user.role !== role.ADMIN && { instructor_id: user.id })
  }, [handler.navigate, user])

  return handler.componentLoaded && <DashboardPageCompement title={"all exams"}>
    <ActionButton name={'add exam'} route={'./add'} />

    {!isNullOrEmpty(exams) ? <Table
      ths={<>
      ...
      tds={exams.map((exam, index) => (
        <tr key={exam.id}>
          ...
          <td className="flex gap-2">
            ...
            <ActionButton
              name={"Delete"}
              onClick={() => destroy("/exams", exam.id, "exam", setExams)}
